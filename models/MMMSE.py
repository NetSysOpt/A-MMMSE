...
class MMMSE():
    def __init__(self, T, R, I, d, sigma, total_power, alpha, max_ite, tol):
        """
    ...
    def forward(self, H, V, rel_rate):
        ...
        if rel_rate <= self.tol:
            ...
            V = np.zeros(shape=(self.T, self.d, self.I)) + 1j * np.zeros(shape=(self.T, self.d, self.I))
            for j in range(self.I):
                J = J + self.alpha[j, 0] * (np.conjugate(H[:, :, j]).T) @ U[:, :, j] @ (np.conjugate(U[:, :, j]).T) @ H[:, :, j]
            ...
            while True:
                ...
                for i in range(self.I):
                    V_tem = np.linalg.inv(J + mu1 * np.eye(self.T)) @ (self.alpha[i, 0] * (np.conjugate(H[:, :, i]).T @ U[:, :, i]))
                    P_tem = P_tem + np.real(np.trace(V_tem @ np.conjugate(V_tem).T))
            ...
            for i in range(self.I):
                V[:, :, i] = np.linalg.inv(J + mu * np.eye(self.T)) @ (self.alpha[i, 0] * (np.conjugate(H[:, :, i]).T @ U[:, :, i] ))
