...
class R_MMMSE():
    ...
    def forward(self, H_hat, X, rel_rate):
        # compute U
        ...
        if rel_rate <= self.tol:
            ...
            # compute X
            eta = self.alpha[0, 0] * np.trace(U[:, :, 0]@ np.conjugate(U[:, :, 0]).T)
            U_hat = U[:, :, 0]

            for i in range(1, self.I):
                U_hat = block_diag(U_hat, U[:, :, i])
                eta = eta + self.alpha[i, 0] * np.trace(U[:, :, i] @ np.conjugate(U[:, :, i]).T)

            eta = eta * (self.sigma / self.P)
            X_hat = U_hat @ np.linalg.inv(eta * np.eye(self.I*self.d) + np.conjugate(U_hat).T @ H_hat @ U_hat)
